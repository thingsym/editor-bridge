...
test.describe( '@editor-bridge expansion', () => {
	...
	test( 'Border expansion', async ( { editor, page } ) => {
		...
		if ( await page.getByRole( 'tabpanel' ).getByRole( 'button', { name: 'Border settings' } ).getAttribute( 'aria-expanded' ) == 'false' ) {
			await page
				.getByRole( 'tabpanel' )
				.getByRole( 'button', { name: 'Border settings', expanded: false } )
				.click();
		}

		const panelBody = await page
			.getByRole( 'tabpanel' )
			.getByRole( 'button', { name: 'Border settings' } )
			.locator( '../..' );
		...
		await expect(
			panelBody
				.getByRole( 'spinbutton', { name: 'Border width' } )
		).toHaveValue( '1' );
...
<!-- /wp:paragraph -->`
		...
		const borderWidthSliderRect = await panelBody
			.getByRole( 'slider', { name: 'Border width' } )
			.boundingBox();
		await panelBody
			.getByRole( 'slider', { name: 'Border width' } )
			.click({
				position: {
					x: borderWidthSliderRect.width /  10 * 6,
					y: borderWidthSliderRect.height * 0.5
				},
			});
...
<!-- /wp:paragraph -->`
		...
		const borderRect = await page
			.locator( '.components-range-control__wrapper' )
			.getByLabel( 'Border width' )
			.boundingBox();
		await page
			.locator( '.components-range-control__wrapper' )
			.getByLabel( 'Border width' )
			.click({
				position: {
					x: borderRect.width * 0.6 + sliderRectOffSet,
					y: borderRect.height * 0.5
				},
			});
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await panelBody
			.getByLabel( 'Border width', { exact: true } )
			.locator('../..')
...
<!-- /wp:paragraph -->`
		...
		// radius
		await panelBody.scrollIntoViewIfNeeded();
		const borderRadiusSliderRect = await panelBody
			.getByRole( 'slider', { name: 'Border radius' } )
			.boundingBox();
		await panelBody
			.getByRole( 'slider', { name: 'Border radius' } )
			.click({
				position: {
					x: borderRadiusSliderRect.width / 100 * 20 + sliderRectOffSet,
					y: borderRadiusSliderRect.height * 0.5
				},
			});
		await expect(
			panelBody
...
<!-- /wp:paragraph -->`
		...
		await panelBody
			.getByLabel( 'Border radius', { exact: true } )
			.locator('../..')
			.getByRole( 'button', { name: 'Reset' } )
			.click();
		await expect(
...
<!-- /wp:paragraph -->`
		...
		await page
			...
			.getByRole( 'menuitem', { name: 'Copy ⌘C', exact: true } )
			.click();
...
<!-- /wp:paragraph -->`
	...
	test( 'Space expansion', async ( { editor, page } ) => {
		...
		if ( await page.getByRole( 'tabpanel' ).getByRole( 'button', { name: 'Space Settings' } ).getAttribute( 'aria-expanded' ) == 'false' ) {
			await page
				.getByRole( 'tabpanel' )
				.getByRole( 'button', { name: 'Space Settings', expanded: false } )
				.click();
		}
...
<!-- /wp:paragraph -->`
		...
		await panelBody
			.getByLabel( 'Disable the horizontal setting' )
			.check();
...
<!-- /wp:paragraph -->`
	...
	test( 'background image expansion', async ( { editor, page } ) => {
		...
		await page
			.getByLabel( 'Block tools' )
			.getByLabel( 'Edit Background Image', { exact: true } )
			.click();
		await page.waitForTimeout(1000);
	...
	test( 'container expansion', async ( { editor, page } ) => {
		...
		await expect(
			page
				...
				.getByRole( 'option', { name: 'Group', selected: true } )
		).toBeVisible();
		await page.keyboard.press( 'Enter' );
		await editor.canvas
			.getByRole( 'list', { name: 'Block variations' } )
			.getByLabel( 'Group: Gather blocks in a container.' )
			.click();

		await editor.canvas
			.locator( '.block-editor-inserter' )
		...
		await page
			...
			.click();
		await editor.canvas
			.getByLabel( 'Empty block; start writing or type forward slash to choose a block' )
			.fill( dummyText );
...
<!-- /wp:group -->`
		...
		await expect(
			page
				.getByRole( 'tabpanel' )
				.getByRole( 'button', { name: 'Container Settings' } )
		).toBeHidden();
		...
		if ( await page.getByRole( 'tabpanel' ).getByRole( 'button', { name: 'Container Settings' } ).getAttribute( 'aria-expanded' ) == 'false' ) {
			await page
				.getByRole( 'tabpanel' )
				.getByRole( 'button', { name: 'Container Settings', expanded: false } )
				.click();
		}
...
<!-- /wp:group -->`
		...
		if ( await page.getByRole( 'tabpanel' ).getByRole( 'button', { name: 'Width Settings' } ).getAttribute( 'aria-expanded' ) == 'false' ) {
			await page
				.getByRole( 'tabpanel' )
				.getByRole( 'button', { name: 'Width Settings', expanded: false } )
				.click();
		}

		const panelBody = await page
			.getByRole( 'tabpanel' )
			.getByRole( 'button', { name: 'Width settings' } )
			.locator( '../..' );

		await panelBody
			.getByRole( 'group', { name: 'Button width' } )
...
<!-- /wp:group -->`
	...
	test( 'block-centered-alignment expansion', async ( { editor, page } ) => {
		...
		await expect(
			page
				.locator( '.components-autocomplete__popover' )
				.getByRole( 'option', { name: 'Columns', selected: true } )
		).toBeVisible();
		await page.keyboard.press( 'Enter' );
		await editor.canvas
		.getByRole( 'list', { name: 'Block variations' } )
		.getByLabel( 'Two columns; equal split' )
		.click();
		...
		await page
			...
			.getByRole( 'option', { name: 'Paragraph' } )
			.click();
		// await editor.canvas.locator( 'p[data-type="core/paragraph"]' ).fill( 'a' );
		await editor.canvas
			.getByRole( 'document', { name: 'Empty block; start writing or type forward slash to choose a block' } )
			.fill( 'a' );

		await editor.canvas
			.getByLabel( 'Block: Column (2 of 2)' )
			.click();
		await editor.canvas
			.getByLabel( 'Block: Column (2 of 2)' )
			.locator( '.block-editor-inserter' )
		...
		await page
			...
			.getByRole( 'option', { name: 'Paragraph' } )
			.click();
		// await editor.canvas.locator( 'p[data-type="core/paragraph"]' ).fill( dummyText );
		await editor.canvas
			.getByRole( 'document', { name: 'Empty block; start writing or type forward slash to choose a block' } )
			.fill( dummyText );
		...
		await page
			.getByRole( 'treegrid', { name: 'Block navigation structure' } )
			.getByRole( 'link', { name: 'Column', exact: true } )
			.nth(0)
			.click();
		await page
			.getByRole( 'region', { name: 'Document Overview' } )
		...
		await page
			.getByLabel( 'Block tools' )
			.getByLabel( 'Change centered alignment', { exact: true } )
			.click();
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:columns -->
...
<!-- /wp:columns -->`
	...
	test( 'button-size expansion', async ( { editor, page } ) => {
		...
		await expect(
			page
				.locator( '.components-autocomplete__popover' )
				.getByRole( 'option', { name: 'Buttons', selected: true } )
		).toBeVisible();
		await page.keyboard.press( 'Enter' );

		await editor.canvas
			.getByRole( 'document', { name: 'Block: Buttons', exact: true } )
		...
		if ( await page.getByRole( 'tabpanel' ).getByRole( 'button', { name: 'Size Settings' } ).getAttribute( 'aria-expanded' ) == 'false' ) {
			await page
				.getByRole( 'tabpanel' )
				.getByRole( 'button', { name: 'Size Settings', expanded: false } )
				.click();
		}

		const panelBody = await page
			.getByRole( 'tabpanel' )
			.getByRole( 'button', { name: 'Size settings' } )
			.locator( '../..' );
		await panelBody
			.getByLabel( 'Size' )
			.selectOption( 'Large' );
...
<div class="wp-block-button is-button-size-large is-button-width-half"><a class="wp-block-button__link wp-element-button">test 1</a></div>
<!-- /wp:button -->
...
<!-- /wp:buttons -->`
	...
	test( 'icon expansion', async ( { editor, page } ) => {
		...
		await editor.canvas
			.getByRole( 'document', { name: 'Block: List', exact: true } )
		...
		// await editor.canvas.getByRole( 'document', { name: 'Block: List', exact: true } ).click();
		await page
			.getByRole( 'toolbar', { name: 'Document tools' } )
			.getByRole( 'button', { name: 'Document Overview' } )
			.click();
		...
		await expect(
			page
				.getByRole( 'tabpanel' )
				.getByRole( 'button', { name: 'Icon Settings' } )
				.isHidden()
		).toBeTruthy();
		...
		await page
			.getByLabel( 'Editor settings' )
			.locator( '.block-editor-block-styles' )
		...
		if ( await page.getByRole( 'tabpanel' ).getByRole( 'button', { name: 'Icon Settings' } ).getAttribute( 'aria-expanded' ) == 'false' ) {
			await page
				.getByRole( 'tabpanel' )
				.getByRole( 'button', { name: 'Icon Settings', expanded: false } )
				.click();
		}
		await page.waitForTimeout(1000);
