...
test.describe( '@default-editor insert block', () => {
	...
	test( 'insert block via quick inserter / search', async ({ editor, page }) => {
		...
		await expect(
			page
				.locator( '.block-editor-inserter__quick-inserter' )
				.getByLabel( 'Blocks', { exact: true } )
	...
	test( 'insert block via quick inserter / Browse all', async ({ editor, page }) => {
		...
		await page
			.locator( '.block-editor-inserter__quick-inserter' )
			.getByText( 'Browse all', { exact: true } )
			.click();

		await expect(
			page
				.locator( '.block-editor-inserter__panel-content' )
	...
	test( 'insert block via Toggle block inserter / search', async ({ editor, page }) => {
		await page
			.getByLabel( 'Toggle block inserter', { exact: true } )
			.click();
		await page.waitForTimeout(1000);
		await page
			.locator( '.block-editor-inserter__tablist' )
			.getByRole( 'tab', { name: 'Blocks' } )
			.click();
		await page
			.locator( '.block-editor-inserter__search' )
	...
	test( 'insert block via Toggle block inserter / drag and drop', async ({ editor, page }) => {
		...
		await expect(
			editor.canvas.locator( 'p[data-type="core/paragraph"]' )
		).toBeVisible();

		const targetRect = await editor.canvas
		...
		const targetRect1 = await editor.canvas
				.locator( 'h2[data-type="core/heading"]' )
				.first()
				.boundingBox();
		...
		await page.mouse.move(
			sourceRect1.x + sourceRect1.width / 2,
			sourceRect1.y + sourceRect1.height / 2
		);
		await page.mouse.down();
		await page.mouse.move(
			targetRect1.x + targetRect1.width / 2,
			targetRect1.y
		);
		await page.mouse.up();
		...
		const targetRect2 = await editor.canvas
				.locator( 'h2[data-type="core/heading"]' )
				.first()
				.boundingBox();
		await page
		...
		await page.mouse.down();
		await page.mouse.move(
			targetRect2.x + targetRect2.width / 2,
			targetRect2.y + targetRect2.height - 12
		);
		await page.mouse.up();
	...
	test.skip( 'insert block via insertion point inserter', async ({ editor, page }) => {
		...
		const paragraphRect = await editor.canvas
			.locator( 'p[data-type="core/paragraph"]' )
			.first()
			.boundingBox();
		// console.log(paragraphRect);
		await page.mouse.click(
			paragraphRect.x + paragraphRect.width / 2,
			paragraphRect.y + paragraphRect.height + 12
		);
		await page.waitForTimeout(1000);
		await page.mouse.click(
			paragraphRect.x + paragraphRect.width / 2,
		...
		await page
			.locator( '.block-editor-block-list__insertion-point-inserter > .block-editor-inserter' )
	...
	test( 'insert block via settings menu', async ({ editor, page }) => {
		...
		await page
			...
			.click();
		await page
			.getByRole( 'menuitem', { name: 'Add before ⌥⌘T', exact: true } )
			.click();
		...
		await page
			...
			.click();
		await page
			.getByRole( 'menuitem', { name: 'Add after ⌥⌘Y', exact: true } )
			.click();
