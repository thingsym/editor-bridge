...
test.describe( '@editor-bridge format', () => {
	...
	test( 'font-size format', async ( { editor, page } ) => {
		...
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitemcheckbox', { name: 'Font Size' } ).click();

		await page.locator( '.components-fontsize-picker' ).getByRole( 'radiogroup', { name: 'Font size' } ).getByRole( 'radio', { name: 'Large', exact: true } ).click();
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.components-fontsize-picker' ).getByRole( 'radiogroup', { name: 'Font size' } ).getByRole( 'radio', { name: 'Extra Extra Large', exact: true } ).click();
		await expect( await editor.getEditedPostContent() ).toContain(
			'<span style="font-size: 10rem;" class="editor-bridge-has-font-size">ipsum</span>'
		);

		await page.locator( '.components-fontsize-picker' ).getByLabel( 'Set custom size' ).click();

		await page.locator( '.components-fontsize-picker' ).getByLabel( 'Custom', { exact: true } ).fill( '3' );
		await expect( await editor.getEditedPostContent() ).toContain(
			'<span style="font-size: 3rem;" class="editor-bridge-has-font-size">ipsum</span>'
		);

		await page.locator( '.components-fontsize-picker' ).getByLabel( 'Select unit' ).selectOption( 'em' );
		await expect( await editor.getEditedPostContent() ).toContain(
			'<span style="font-size: 3em;" class="editor-bridge-has-font-size">ipsum</span>'
		);

		// Reset
		await page.locator( '.components-fontsize-picker' ).getByRole( 'button', { name: 'Reset', exact: true } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:paragraph -->
<p>${dummyText}</p>
<!-- /wp:paragraph -->`
	...
	test( 'badge format', async ( { editor, page } ) => {
		...
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitemcheckbox', { name: 'Badge' } ).click();
		await page.waitForTimeout(1000);

		// ColorPicker
		await page.locator( '.components-circular-option-picker__swatches' ).getByLabel( 'Color: Secondary' ).click();
		await page.locator( '.components-inline-badge-popover' ).getByRole( 'tablist' ).getByRole( 'tab', { name: 'Text' } ).click();
		await page.locator( '.components-circular-option-picker__swatches' ).getByLabel( 'Color: Tertiary' ).click();
		await page.locator( '.components-inline-badge-popover' ).getByRole( 'tablist' ).getByRole( 'tab', { name: 'Style' } ).click();
		await page.locator( '.components-select-control__input' ).selectOption( 'round' );
		await expect( await editor.getEditedPostContent() ).toContain(
		...
		await page.locator( '.components-inline-badge-popover' ).getByRole( 'tablist' ).getByRole( 'tab', { name: 'Background' } ).click();
		...
		// Reset
		await page.locator( '.components-inline-badge-popover' ).getByRole( 'tablist' ).getByRole( 'tab', { name: 'Style' } ).click();
		await page.locator( '.components-inline-badge-popover' ).getByRole( 'tabpanel' ).getByRole( 'button', { name: 'Reset', exact: true } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:paragraph -->
<p>${dummyText}</p>
<!-- /wp:paragraph -->`
	...
	test( 'font-weight format', async ( { editor, page } ) => {
		...
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitemcheckbox', { name: 'Font Weight' } ).click();
		await page.waitForTimeout(1000);

		await page.locator( '.components-select-control__input' ).selectOption( 'bold' );
		await expect( await editor.getEditedPostContent() ).toContain(
			'<span style="font-weight: bold;" class="editor-bridge-has-font-weight">ipsum</span>'
		);

		// Reset
		await page.locator( '.components-inline-fontweight-popover' ).getByRole( 'button', { name: 'Reset', exact: true } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:paragraph -->
<p>${dummyText}</p>
<!-- /wp:paragraph -->`
	...
	test( 'highlight format', async ( { editor, page } ) => {
		...
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitemcheckbox', { name: 'Highlight' } ).nth(1).click();
		await page.waitForTimeout(1000);

		// Style
		await page.locator( '.components-circular-option-picker__swatches' ).getByLabel( 'Color: Secondary' ).click();
		await page.locator( '.components-inline-highligh-popover' ).getByRole( 'tablist' ).getByRole( 'tab', { name: 'Style' } ).click();
		await page.locator( '.components-select-control__input' ).selectOption( 'marker' );
		await expect( await editor.getEditedPostContent() ).toContain(
		...
		await page.locator( '.components-inline-highligh-popover' ).getByRole( 'tablist' ).getByRole( 'tab', { name: 'Color' } ).click();
		await page.locator( '.components-color-palette__custom-color-wrapper' ).getByRole( 'button', { name: 'Custom color picker.' } ).click();
		await page.waitForTimeout(3000);
