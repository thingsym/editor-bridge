...
<!-- /wp:paragraph -->`
	...
	test( 'default panel / text', async ({ editor, page }) => {
		...
		await page.locator( '.color-block-support-panel > .color-block-support-panel__inner-wrapper' ).getByRole( 'button', { name: 'Color Text styles' } ).click();

		await page.locator( '.components-circular-option-picker__swatches' ).getByRole( 'option', { name: 'Color: Vivid red' } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.components-circular-option-picker__swatches' ).getByRole( 'option', { name: 'Color: Vivid red' } ).click();
		await page.locator( '.components-circular-option-picker__custom-clear-wrapper' ).getByRole( 'button', { name: 'Clear' } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:paragraph -->
<p>${dummyText}</p>
<!-- /wp:paragraph -->`
		...
		await page.locator( '.components-circular-option-picker__swatches' ).getByRole( 'option', { name: 'Color: Vivid red' } ).click();
		await page.locator( '.color-block-support-panel' ).getByRole( 'button', { name: 'Color options' } ).click();
		await page.getByRole( 'menu', { name: 'Color options' } ).getByRole( 'menuitem', { name: 'Reset Text' } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:paragraph -->
<p>${dummyText}</p>
<!-- /wp:paragraph -->`
		...
		const alphaTextRect = await page.locator( '.react-colorful__alpha' ).getByRole( 'slider', { name: 'Alpha' } ).boundingBox();
		await page.locator( '.react-colorful__alpha' ).getByRole( 'slider', { name: 'Alpha' } ).click({
			position: { x: alphaTextRect.width * 0.2, y: alphaTextRect.height * 0.5 },
		});
...
<!-- /wp:paragraph -->`
		...
		await page.getByRole( 'spinbutton', { name: 'Alpha', exact: true } ).fill( '20' );
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.getByRole( 'spinbutton', { name: 'Alpha', exact: true } ).fill( '80' );
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.components-circular-option-picker__swatches' ).getByRole( 'option', { name: 'Color: Pale pink' } ).click();
		await page.locator( '.color-block-support-panel' ).getByRole( 'button', { name: 'Color options' } ).click();
		await page.getByRole( 'menu', { name: 'Color options' } ).getByRole( 'menuitem', { name: 'Reset Background' } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:paragraph -->
<p>${dummyText}</p>
<!-- /wp:paragraph -->`
		...
		await page.waitForTimeout(3000);
		const saturationBackgroundRect = await page.locator( '.react-colorful__saturation' ).getByRole( 'slider', { name: 'Color' } ).boundingBox();
		await page.locator( '.react-colorful__saturation' ).getByRole( 'slider', { name: 'Color' } ).click({
			position: { x: Number( saturationBackgroundRect.width ) * 0.2, y: Number( saturationBackgroundRect.height ) * 0.2 },
		});

		const hueBackgroundRect = await page.locator( '.react-colorful__hue' ).getByRole( 'slider', { name: 'Hue' } ).boundingBox();
		await page.locator( '.react-colorful__hue' ).getByRole( 'slider', { name: 'Hue' } ).click({
			position: { x: hueBackgroundRect.width * 0.2, y: hueBackgroundRect.height * 0.5 },
		});

		const alphaBackgroundRect = await page.locator( '.react-colorful__alpha' ).getByRole( 'slider', { name: 'Alpha' } ).boundingBox();
		await page.locator( '.react-colorful__alpha' ).getByRole( 'slider', { name: 'Alpha' } ).click({
			position: { x: alphaBackgroundRect.width * 0.2, y: alphaBackgroundRect.height * 0.5 },
		});
...
<!-- /wp:paragraph -->`
		...
		await pointerBackgroundSource.dragTo( pointerBackgroundTarget, {
			targetPosition: { x: Number( saturationBackgroundRect.width ) * 0.4, y: Number( saturationBackgroundRect.height ) * 0.4 },
		});
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
	...
	test( 'default panel / gradient background', async ({ editor, page }) => {
		...
		await page.getByLabel( 'Editor settings' ).locator( '.block-editor-block-inspector__tabs' ).getByRole( 'tab', { name: 'Styles' } ).click();
		await page.locator( '.color-block-support-panel > .color-block-support-panel__inner-wrapper' ).getByRole( 'button', { name: 'Color Background styles' } ).click();
		await page.locator( '.block-editor-panel-color-gradient-settings__dropdown-content' ).getByRole( 'tablist' ).getByRole( 'tab', { name: 'Gradient' } ).click();
		...
		await page.locator( '.components-circular-option-picker__swatches' ).getByLabel( 'Gradient: Luminous vivid orange to vivid red' ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.components-circular-option-picker__swatches' ).getByRole( 'option', { name: 'Gradient: Luminous vivid orange to vivid red' } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:paragraph -->
<p>${dummyText}</p>
<!-- /wp:paragraph -->`
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.components-circular-option-picker__swatches' ).getByLabel( 'Gradient: Luminous vivid orange to vivid red' ).click();
		await page.locator( '.components-custom-gradient-picker__ui-line' ).getByLabel( 'Type' ).selectOption( 'radial-gradient' );
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.components-custom-gradient-picker__ui-line' ).getByLabel( 'Type' ).selectOption( 'linear-gradient' );
		await page.locator( '.components-custom-gradient-picker__ui-line' ).getByLabel( 'Angle' ).fill( '60' );
...
<!-- /wp:paragraph -->`
		...
		const angleCircleRect = await page.locator( '.components-angle-picker-control__angle-circle' ).boundingBox();
		...
		await angleCircleSource.dragTo( angleCircleTarget, {
			targetPosition: { x: Number( angleCircleRect.width ) * 0.5 - 1, y: Number( angleCircleRect.height ) * 0 + 1 },
		});
		await page.waitForTimeout(3000);
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.color-block-support-panel > .color-block-support-panel__inner-wrapper' ).getByRole( 'button', { name: 'Color Background styles' } ).click();
		await page.locator( '.block-editor-panel-color-gradient-settings__dropdown-content' ).getByRole( 'tablist' ).getByRole( 'tab', { name: 'Gradient' } ).click();
		await page.locator( '.components-circular-option-picker__swatches' ).getByRole( 'option', { name: 'Gradient: Luminous vivid amber to luminous vivid orange' } ).click();

		// palette
		await page.getByLabel( /Gradient control point at position/ ).first().click();
		await page.waitForTimeout(3000);
		const saturationGradientFirestRect = await page.locator( '.react-colorful__saturation' ).getByRole( 'slider', { name: 'Color' } ).boundingBox();
		await page.locator( '.react-colorful__saturation' ).getByRole( 'slider', { name: 'Color' } ).click({
			position: { x: Number( saturationGradientFirestRect.width ) * 0.2, y: Number( saturationGradientFirestRect.height ) * 0.2 },
		});

		const hueGradientFirestRect = await page.locator( '.react-colorful__hue' ).getByRole( 'slider', { name: 'Hue' } ).boundingBox();
		await page.locator( '.react-colorful__hue' ).getByRole( 'slider', { name: 'Hue' } ).click({
			position: { x: hueGradientFirestRect.width * 0.2, y: hueGradientFirestRect.height * 0.5 },
		});

		const alphaGradientFirestRect = await page.locator( '.react-colorful__alpha' ).getByRole( 'slider', { name: 'Alpha' } ).boundingBox();
		await page.locator( '.react-colorful__alpha' ).getByRole( 'slider', { name: 'Alpha' } ).click({
			position: { x: alphaGradientFirestRect.width * 0.2, y: alphaGradientFirestRect.height * 0.5 },
		});

		await page.getByLabel( /Gradient control point at position/ ).last().click();
		await page.waitForTimeout(3000);

		const saturationGradientLastRect = await page.locator( '.react-colorful__saturation' ).getByRole( 'slider', { name: 'Color' } ).boundingBox();
		await page.locator( '.react-colorful__saturation' ).getByRole( 'slider', { name: 'Color' } ).click({
			position: { x: Number( saturationGradientLastRect.width ) * 0.8, y: Number( saturationGradientLastRect.height ) * 0.2 },
		});

		const hueGradientLastRect = await page.locator( '.react-colorful__hue' ).getByRole( 'slider', { name: 'Hue' } ).boundingBox();
		await page.locator( '.react-colorful__hue' ).getByRole( 'slider', { name: 'Hue' } ).click({
			position: { x: hueGradientLastRect.width * 0.8, y: hueGradientLastRect.height * 0.8 },
		});

		const alphaGradientLastRect = await page.locator( '.react-colorful__alpha' ).getByRole( 'slider', { name: 'Alpha' } ).boundingBox();
		await page.locator( '.react-colorful__alpha' ).getByRole( 'slider', { name: 'Alpha' } ).click({
			position: { x: alphaGradientLastRect.width * 0.8, y: alphaGradientLastRect.height * 0.8 },
		});
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.block-editor-color-gradient-control__panel' ).click();
		await page.getByLabel( /Gradient control point at position/ ).first().click();
		await page.waitForTimeout(3000);
		...
		await pointerGradientSource.dragTo( pointerGradientTarget, {
			targetPosition: { x: Number( saturationGradientFirestRect.width ) * 0.4, y: Number( saturationGradientFirestRect.height ) * 0.4 },
		});
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.block-editor-color-gradient-control__panel' ).click();
		const gradientPickerRect = await page.locator( '.components-custom-gradient-picker__markers-container' ).boundingBox();
		const slideTarget = gradientPickerRect.x + gradientPickerRect.width / 2;

		// add gradient point
		// linear-color-stop is 50%
		await page.mouse.click( slideTarget, gradientPickerRect.y + 20 );

		await page.locator( '.components-custom-gradient-picker__insert-point-dropdown' ).click();
		await page.getByLabel( 'Color format' ).selectOption( 'Hex' );
		await page.getByLabel( 'Hex color', { exact: true } ).fill( '#da0c0c' );
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		// remove point
		await page.locator( '.block-editor-color-gradient-control__panel' ).click();
		await page.getByLabel( /Gradient control point at position/ ).nth(1).click();
		await page.locator( '.components-color-palette__custom-color-dropdown-content' ).scrollIntoViewIfNeeded();
		await page.locator( '.components-color-palette__custom-color-dropdown-content' ).getByRole( 'button', { name: 'Remove Control Point' } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:paragraph {"style":{"color":{"gradient":"linear-gradient(135deg,rgba(50,100,200,0.8) 0%,rgba(166,41,204,0.8) 100%)"}}} -->
<p class="has-background" style="background:linear-gradient(135deg,rgba(50,100,200,0.8) 0%,rgba(166,41,204,0.8) 100%)">${dummyText}</p>
<!-- /wp:paragraph -->`
		...
		// linear-color-stop is 50%
		await page.locator( '.block-editor-color-gradient-control__panel' ).click();
		const gradientPickerPointRect = await page.getByLabel( /Gradient control point at position/ ).first().boundingBox();
		await page.getByLabel( /Gradient control point at position/ ).first().hover();

		await page.mouse.down();
		await page.mouse.move( slideTarget, gradientPickerPointRect.y + 24 );
		await page.mouse.up();
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
	...
	test( 'default panel / typography', async ({ editor, page }) => {
		...
		await page.getByLabel( 'Editor settings' ).locator( '.block-editor-block-inspector__tabs' ).getByRole( 'tab', { name: 'Styles' } ).click();
		await page.locator( '.typography-block-support-panel' ).getByLabel ( 'Large', { exact: true } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.typography-block-support-panel' ).getByRole( 'button', { name: 'Set custom size' } ).click();
		await page.locator( '.typography-block-support-panel' ).getByLabel ( 'Custom', { exact: true } ).fill( '4' );
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.getByLabel( 'Custom', { exact: true } ).locator( '../..' ).getByLabel( 'Select unit' ).selectOption( 'px' );
		await page.locator( '.typography-block-support-panel' ).getByLabel ( 'Custom', { exact: true } ).fill( '24' );
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
	...
	test( 'support panel', async ({ editor, page }) => {
		...
		await page.getByRole( 'menu', { name: 'Color options' } ).getByRole( 'menuitemcheckbox', { name: 'Show Link' } ).click();

		await page.locator( 'p[data-type="core/paragraph"]' ).click();

		await page.locator( '.typography-block-support-panel' ).getByRole( 'button', { name: 'Typography options' } ).click();

		await page.getByRole( 'menu', { name: 'Typography options' } ).getByRole( 'menuitemcheckbox', { name: 'Font' } ).click();
		await page.getByRole( 'menu', { name: 'Typography options' } ).getByRole( 'menuitemcheckbox', { name: 'Appearance' } ).click();
		await page.getByRole( 'menu', { name: 'Typography options' } ).getByRole( 'menuitemcheckbox', { name: 'Line height' } ).click();
		await page.getByRole( 'menu', { name: 'Typography options' } ).getByRole( 'menuitemcheckbox', { name: 'Letter spacing' } ).click();
		await page.getByRole( 'menu', { name: 'Typography options' } ).getByRole( 'menuitemcheckbox', { name: 'Decoration' } ).click();
		await page.getByRole( 'menu', { name: 'Typography options' } ).getByRole( 'menuitemcheckbox', { name: 'Letter case' } ).click();
		...
		await page.locator( '.dimensions-block-support-panel' ).getByRole( 'button', { name: 'Dimensions options' } ).click();

		await page.getByRole( 'menu', { name: 'Dimensions options' } ).getByRole( 'menuitemcheckbox', { name: 'Padding' } ).click();
		await page.getByRole( 'menu', { name: 'Dimensions options' } ).getByRole( 'menuitemcheckbox', { name: 'Margin' } ).click();
		...
		if ( await page.getByRole( 'button', { name: 'Advanced' } ).getAttribute( 'aria-expanded' ) == 'false' ) {
			await page.getByRole( 'button', { name: 'Advanced', expanded: false } ).click();
		}
	...
	test( 'color panel', async ({ editor, page }) => {
		...
		await page.locator( 'p[data-type="core/paragraph"]' ).selectText();
		await page.getByLabel( 'Block tools' ).getByLabel( 'Link', { exact: true } ).click();
		await page.getByPlaceholder( 'Search or type url' ).fill( 'test.html' );
		await page.getByRole( 'option', { name: 'test.html Press ENTER to add this link link' } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:paragraph -->
...
<!-- /wp:paragraph -->`
		...
		await expect( page.locator( '.color-block-support-panel > .color-block-support-panel__inner-wrapper' ).getByRole( 'button', { name: 'Color Link styles' } ).getByLabel( 'Link' ).isVisible() ).toBeTruthy();

		await page.locator( '.color-block-support-panel > .color-block-support-panel__inner-wrapper' ).getByRole( 'button', { name: 'Color Link styles' } ).click();

		await page.locator( '.components-circular-option-picker__swatches' ).getByRole( 'option', { name: 'Color: Vivid cyan blue' } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.block-editor-panel-color-gradient-settings__dropdown-content' ).getByRole( 'tablist' ).getByRole( 'tab', { name: 'Hover' } ).click();
		await page.locator( '.components-circular-option-picker__swatches' ).getByRole( 'option', { name: 'Color: Vivid red' } ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		// await page.locator( 'p[data-type="core/paragraph"]' ).click({ position: { x: 300, y: 0 } });
		// await page.locator( 'p[data-type="core/paragraph"]' ).hover({ position: { x: 300, y: 10 } });
	...
	test( 'typography panel', async ({ editor, page }) => {
		...
		await expect( page.locator( '.typography-block-support-panel' ).getByText( 'Font', { exact: true } ).isVisible() ).toBeTruthy();
		await expect( page.locator( '.typography-block-support-panel' ).getByText( 'Appearance', { exact: true } ).isVisible() ).toBeTruthy();
		await expect( page.locator( '.typography-block-support-panel' ).getByText( 'Line height', { exact: true } ).isVisible() ).toBeTruthy();
		await expect( page.locator( '.typography-block-support-panel' ).getByText( 'Letter spacing', { exact: true } ).isVisible() ).toBeTruthy();
		await expect( page.locator( '.typography-block-support-panel' ).getByText( 'Decoration', { exact: true } ).isVisible() ).toBeTruthy();
		await expect( page.locator( '.typography-block-support-panel' ).getByText( 'Letter case', { exact: true } ).isVisible() ).toBeTruthy();

		await page.getByLabel( 'Font', { exact: true } ).selectOption( '"Source Serif Pro", serif' );
...
// 		);
		...
		await page.getByLabel( 'Line height', { exact: true } ).fill( '1.9' );
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( 'body' ).click();await page.locator( '.block-editor-line-height-control' ).getByLabel( 'Increment' ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.block-editor-line-height-control' ).getByLabel( 'Decrement' ).click();
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:paragraph {"style":{"typography":{"lineHeight":"1.9"}},"fontFamily":"source-serif-pro"} -->
<p class="has-source-serif-pro-font-family" style="line-height:1.9">${dummyText}</p>
<!-- /wp:paragraph -->`
...
<!-- /wp:paragraph -->`
		...
		await page.getByLabel( 'Letter spacing', { exact: true } ).locator( '../..' ).getByLabel( 'Select unit' ).selectOption( 'rem' );
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.getByLabel( 'Letter spacing', { exact: true } ).locator( '../..' ).getByLabel( 'Select unit' ).selectOption( 'px' );

		// await page.getByRole( 'group', { name: 'Decoration' } ).getByLabel( 'None' ).click();
		await page.getByRole( 'group', { name: 'Decoration' } ).getByLabel( 'Underline' ).click();
...
<!-- /wp:paragraph -->`
		...
		await page.getByRole( 'group', { name: 'Letter case' } ).getByLabel( 'Uppercase' ).click();
...
<!-- /wp:paragraph -->`
	...
	test( 'dimensions panel', async ({ editor, page }) => {
		...
		await expect( page.locator( '.dimensions-block-support-panel' ).getByText( 'Padding', { exact: true } ).isVisible() ).toBeTruthy();
		await expect( page.locator( '.dimensions-block-support-panel' ).getByText( 'Margin', { exact: true } ).isVisible() ).toBeTruthy();

		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'padding', { exact: true } ).nth(0).fill( '2' );
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'padding', { exact: true } ).nth(1).fill( '4' );
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'padding', { exact: true } ).nth(0).locator( '../../../../..' ).getByLabel( 'Set custom size' ).click();
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'padding', { exact: true } ).nth(0).fill( '12' );
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'padding', { exact: true } ).nth(0).locator( '../..' ).getByLabel( 'Select unit' ).selectOption( 'rem' );
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'padding', { exact: true } ).nth(0).locator( '../../../../..' ).getByLabel( 'Use size preset' ).click();
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'padding', { exact: true } ).nth(0).fill( '2' );
		await expect( await editor.getEditedPostContent() ).toBe(
			`<!-- wp:paragraph {"style":{"spacing":{"padding":{"top":"var:preset|spacing|40","bottom":"var:preset|spacing|40","left":"var:preset|spacing|60","right":"var:preset|spacing|60"}}},"backgroundColor":"pale-pink"} -->
<p class="has-pale-pink-background-color has-background" style="padding-top:var(--wp--preset--spacing--40);padding-right:var(--wp--preset--spacing--60);padding-bottom:var(--wp--preset--spacing--40);padding-left:var(--wp--preset--spacing--60)">${dummyText}</p>
<!-- /wp:paragraph -->`
		...
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'margin', { exact: true } ).nth(0).fill( '2' );
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'margin', { exact: true } ).nth(1).fill( '4' );
		await expect( await editor.getEditedPostContent() ).toBe(
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.dimensions-block-support-panel' ).getByRole( 'button', { name: 'Padding options' } ).click();
		await page.getByRole( 'menu', { name: 'Padding options' } ).getByRole( 'menuitemradio', { name: 'Top' } ).click();
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Top padding', { exact: true } ).fill( '3' );
		await page.locator( '.dimensions-block-support-panel' ).getByRole( 'button', { name: 'Padding options' } ).click();
		await page.getByRole( 'menu', { name: 'Padding options' } ).getByRole( 'menuitemradio', { name: 'Right' } ).click();
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Right padding', { exact: true } ).fill( '3' );
		await page.locator( '.dimensions-block-support-panel' ).getByRole( 'button', { name: 'Padding options' } ).click();
		await page.getByRole( 'menu', { name: 'Padding options' } ).getByRole( 'menuitemradio', { name: 'Left' } ).click();
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Left padding', { exact: true } ).fill( '3' );
		await page.locator( '.dimensions-block-support-panel' ).getByRole( 'button', { name: 'Padding options' } ).click();
		await page.getByRole( 'menu', { name: 'Padding options' } ).getByRole( 'menuitemradio', { name: 'Bottom' } ).click();
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Bottom padding', { exact: true } ).fill( '3' );
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.dimensions-block-support-panel' ).getByRole( 'button', { name: 'Padding options' } ).click();
		await page.getByRole( 'menu', { name: 'Padding options' } ).getByRole( 'menuitemradio', { name: 'Custom' } ).click();
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Top padding', { exact: true } ).fill( '5' );
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Right padding', { exact: true } ).fill( '5' );
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Left padding', { exact: true } ).fill( '5' );
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Bottom padding', { exact: true } ).fill( '5' );
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Top padding', { exact: true } ).locator( '../../../../..' ).getByLabel( 'Set custom size' ).click();
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Right padding', { exact: true } ).locator( '../../../../..' ).getByLabel( 'Set custom size' ).click();
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Left padding', { exact: true } ).locator( '../../../../..' ).getByLabel( 'Set custom size' ).click();
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Bottom padding', { exact: true } ).locator( '../../../../..' ).getByLabel( 'Set custom size' ).click();

		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Top padding', { exact: true } ).fill( '24' );
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Right padding', { exact: true } ).fill( '24' );
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Left padding', { exact: true } ).fill( '24' );
		await page.locator( '.dimensions-block-support-panel' ).getByLabel( 'Bottom padding', { exact: true } ).fill( '24' );
...
<!-- /wp:paragraph -->`
		...
		await page.locator( '.dimensions-block-support-panel' ).getByRole( 'button', { name: 'Padding options' } ).click();
		await page.getByRole( 'menu', { name: 'Padding options' } ).getByRole( 'menuitemradio', { name: 'Horizontal & vertical' } ).click();
	...
	test( 'advanced panel', async ({ editor, page }) => {
		...
		await expect( page.locator( '.block-editor-block-inspector__advanced' ).getByText( 'HTML anchor', { exact: true } ).isVisible() ).toBeTruthy();
		await expect( page.locator( '.block-editor-block-inspector__advanced' ).getByText( 'Additional CSS class(es)', { exact: true } ).isVisible() ).toBeTruthy();

		await page.getByLabel( 'HTML anchor' ).click();
		await page.getByLabel( 'HTML anchor' ).fill( 'abc' );
		await page.getByLabel( 'Additional CSS class(es)' ).click();
		await page.getByLabel( 'Additional CSS class(es)' ).fill( 'test' );
...
<!-- /wp:paragraph -->`
	...
	test( 'toolbar', async ({ editor, page }) => {
		...
		await page.locator( 'p[data-type="core/paragraph"]' ).selectText();
		await page.locator( 'p[data-type="core/paragraph"]' ).press( 'ArrowLeft' );

		for ( const i in Array(6).fill(0) ) {
			await page.locator( 'p[data-type="core/paragraph"]' ).press( 'ArrowRight' );
		}
		for ( const i in Array(5).fill(0) ) {
			await page.locator( 'p[data-type="core/paragraph"]' ).press( 'Shift+ArrowRight' );
		}
		...
		for ( const i in Array(14).fill(0) ) {
			await page.locator( 'p[data-type="core/paragraph"]' ).press( 'ArrowRight' );
		}
		for ( const i in Array(5).fill(0) ) {
			await page.locator( 'p[data-type="core/paragraph"]' ).press( 'Shift+ArrowRight' );
		...
		// Bold
		await page.getByLabel( 'Block tools' ).getByLabel( 'Bold', { exact: true } ).click();
		await expect( await editor.getEditedPostContent() ).toContain(
			'<strong>dolor</strong>'
		);
		...
		// Italic
		await page.getByLabel( 'Block tools' ).getByLabel( 'Italic', { exact: true } ).click();
		await expect( await editor.getEditedPostContent() ).toContain(
			'<em>amet</em>'
		);
		...
		// Strikethrough
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitemcheckbox', { name: 'Strikethrough' } ).click();
		await expect( await editor.getEditedPostContent() ).toContain(
			'<s>consectetur</s>'
		);
		...
		// Inline code
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitemcheckbox', { name: 'Inline code' } ).click();
		await expect( await editor.getEditedPostContent() ).toContain(
			'<code>adipisci</code>'
		);
		...
		// Keyboard input
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitemcheckbox', { name: 'Keyboard input' } ).click();
		await expect( await editor.getEditedPostContent() ).toContain(
			'<kbd>elit</kbd>'
		);
		...
		// Subscript
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitemcheckbox', { name: 'Subscript' } ).click();
		await expect( await editor.getEditedPostContent() ).toContain(
			'<sub>sed</sub>'
		);
		...
		// Superscript
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitemcheckbox', { name: 'Superscript' } ).click();
		await expect( await editor.getEditedPostContent() ).toContain(
			'<sup>eiusmod</sup>'
		);
		...
		// Highlight
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitemcheckbox', { name: 'Highlight' } ).first().click();

		// ColorPicker
		await page.locator( '.components-circular-option-picker__swatches' ).getByLabel( 'Color: Tertiary' ).click();

		await page.locator( '.format-library__inline-color-popover' ).getByRole( 'tablist' ).getByRole( 'tab', { name: 'Background' } ).click();
		await page.locator( '.components-circular-option-picker__swatches' ).getByLabel( 'Color: Primary' ).click();
		...
		await page.locator( '.format-library__inline-color-popover' ).getByRole( 'tablist' ).getByRole( 'tab', { name: 'Text' } ).click();
		await page.locator( '.components-color-palette__custom-color-wrapper' ).getByRole( 'button', { name: 'Custom color picker.' } ).click();
		await page.waitForTimeout(3000);
		...
		// Language
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitemcheckbox', { name: 'Language' } ).click();

		await page.locator( '.block-editor-format-toolbar__language-popover' ).getByLabel( 'Language' ).fill( 'en' );
		await page.locator( '.block-editor-format-toolbar__language-popover' ).getByLabel( 'Text direction' ).selectOption( 'ltr' );
		await page.locator( '.block-editor-format-toolbar__language-popover' ).getByRole( 'button', { name: 'Apply' } ).click();
		await expect( await editor.getEditedPostContent() ).toContain(
		...
		await page.locator( '.block-editor-format-toolbar__language-popover' ).getByLabel( 'Language' ).fill( 'fr' );
		await page.locator( '.block-editor-format-toolbar__language-popover' ).getByLabel( 'Text direction' ).selectOption( 'rtl' );
		await page.locator( '.block-editor-format-toolbar__language-popover' ).getByRole( 'button', { name: 'Apply' } ).click();
		await expect( await editor.getEditedPostContent() ).toContain(
		...
		// Inline image
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitem', { name: 'Inline image' } ).click();
		...
		// Footnote
		await page.getByLabel( 'Block tools' ).getByLabel( 'More', { exact: true } ).click();
		await page.getByRole( 'menu', { name: 'More' } ).getByRole( 'menuitem', { name: 'Footnote' } ).click();
